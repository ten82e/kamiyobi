...
const TIMEZONE_PATTERN =
  /\b(AoE|UTC(?:[+-]\d{1,2}(?::?\d{2})?)?|GMT(?:[+-]\d{1,2}(?::?\d{2})?)?|PST|PDT|MST|MDT|CST|CDT|EST|EDT|CET|CEST|JST|BOT|COT|FJT|GET|PKT|TRT|BRT|CAT|WAT|NZST|NZDT|WIB|WITA|WIT|IDT|MSK|ChST|CHST|HAST|HADT|HST|EAT|SAST|ACST|ACDT|PT|ET|CT|MT|[A-Za-z_]+(?:\/[A-Za-z_-]+)+)\b/gi;
...
function extractedTimezone(text: string): string | undefined {
  for (const match of text.matchAll(TIMEZONE_PATTERN)) {
    ...
    if (
      /^(?:pt|et|ct|mt|bot|cot|get|cat|wat|wit|eat)$/i.test(candidate) &&
