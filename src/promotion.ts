...
export function extractCfpCandidates(body: string): CfpExtractionCandidate[] {
  ...
  const rawLines = text
    ...
    .filter(Boolean);
  const deadlineLabel =
    /deadline|due|not be entertained after|not entertained after|not be considered after|not considered after|not be accepted after|not accepted after|must arrive|notification|camera[- ]?ready|締切|期限/i;
  ...
  for (const raw of lines) {
    if (
      !/deadline|due|not be entertained after|not entertained after|not be considered after|not considered after|not be accepted after|not accepted after|must arrive|submission|submit|notification|camera[- ]?ready|call for papers|cfp|event|conference|開催|締切|期限|投稿|募集/i.test(
        raw,
      )
    )
      continue;
