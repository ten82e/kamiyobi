...
const TIMEZONE_PATTERN =
  /\b(AoE|UTC(?:[+-]\d{1,2}(?::?\d{2})?)?|GMT(?:[+-]\d{1,2}(?::?\d{2})?)?|PST|PDT|MST|MDT|CST|CDT|EST|EDT|CET|CEST|JST|BOT|PT|ET|CT|MT|[A-Za-z_]+(?:\/[A-Za-z_-]+)+)\b/gi;
...
function extractedTimezone(text: string): string | undefined {
  for (const match of text.matchAll(TIMEZONE_PATTERN)) {
    ...
    if (/^(?:pt|et|ct|mt|bot)$/i.test(candidate) && candidate !== candidate.toUpperCase()) {
      continue;
    }
