...
function extractedTime(text: string): string | undefined {
  ...
  const hourH = /\b(\d{1,2})h([0-5]\d)\b/i.exec(text);
  if (hourH) {
    const hour = Number(hourH[1]);
    const minute = Number(hourH[2]);
...
export function extractCfpCandidates(body: string): CfpExtractionCandidate[] {
  ...
  const hasTimeExpression = (value: string) =>
    ...
    /\b(?:\d{1,2}:\d{2}(?::\d{2})?|\d{1,2}h[0-5]\d|\d{1,2}\s*(?:a\.?m\.?|p\.?m\.?)|at\s+\d{3,4}|noon|midnight|end of (?:the )?day|eod)\b/i.test(
      value,
    );
