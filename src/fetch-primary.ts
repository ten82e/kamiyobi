...
const TAG_RE = /<[^>]+>/g;
const TZ_RE =
  /\b(PDT|PST|EDT|EST|CDT|CST|MDT|MST|AKDT|AKST|HST|HAST|HADT|UTC|GMT|CET|CEST|JST|BOT|COT|FJT|GET|PKT|TRT|BRT|CAT|WAT|NZST|NZDT|WIB|WITA|WIT|IDT|MSK|CHST|EAT|SAST|ACST|ACDT|AoE|PT|ET|CT|MT)\b|anywhere on (?:the )?(?:inhabited )?earth/gi;
...
export function extractDeadline(
  ...
  if (tzM) {
    const raw = tzM[0];
    if (/^(?:bot|cot|get|cat|wat|wit|eat)$/i.test(raw) && raw !== raw.toUpperCase()) {
      tz = undefined;
    } else {
      tz =
