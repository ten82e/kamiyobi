...
describe("promotion batch", () => {
  ...
  it("extracts global deadline timing with multi-segment IANA zones and 12-hour format (#740)", () => {
    ...
    expect(extractCfpCandidates("Paper submission deadline: May 15, 2026 5 PM PDT")).toMatchObject([
      { date: "2026-05-15", time: "17:00:00", timezone: "PDT" },
    ]);
    expect(extractCfpCandidates("Paper deadline: May 15, 2026 5PM AoE")).toMatchObject([
      { date: "2026-05-15", time: "17:00:00", timezone: "AoE" },
    ]);
    expect(extractCfpCandidates("Paper deadline: May 15, 2026 11 p.m. AoE")).toMatchObject([
      { date: "2026-05-15", time: "23:00:00", timezone: "AoE" },
    ]);
    expect(extractCfpCandidates("Paper deadline: May 15, 2026 12 AM UTC")).toMatchObject([
      { date: "2026-05-15", time: "00:00:00", timezone: "UTC" },
    ]);
  });
  ...
  it("does not combine a named row time with the page-wide default", () => {
    for (const rowTime of ["noon UTC", "25:00 UTC", "at 1700 UTC", "1700 UTC", "1700"]) {
    ...
    expect(
      extractCfpCandidates("Paper deadline October 10, 2026 5 PM UTC\nAll deadlines are 23:59 UTC"),
    ).toMatchObject([{ date: "2026-10-10", time: "17:00:00", timezone: "UTC" }]);
  });
